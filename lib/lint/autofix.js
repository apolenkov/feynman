...
// Strip ANSI CSI sequences. Used by visualWidth. Hex-escape the ESC char to
// keep this file free of literal control bytes.
const ANSI_RE = /\x1b\[[0-9;]*m/g;

// Combining marks (Mn) and zero-width joiners that should NOT count toward
// visual width. Expressed as Unicode escapes to avoid invisible-char warnings.
...
function visualWidth(line) {
  const stripped = line.replace(ANSI_RE, '').replace(ZERO_WIDTH_RE, '');
  // Use spread to count grapheme clusters (better than .length for surrogate pairs).

...
// at the same character-column as the top ┐. Detection-only (no autofix).
// Uses codepoint-aware indexing (matches L01 column convention).
// ---------------------------------------------------------------------------
...
function L09_right_edge_alignment(ast) {
  ...
  while (li < lines.length) {
    ...
    if (topLine.includes('┌') && topLine.includes('┐')) {
      const anchorTopCol = charIndexOf(topLine, '┌') + 1;
      const anchorCol    = lastCharIndexOf(topLine, '┐') + 1;
      const topAbsLine   = baseLineNum + li;

      // Find the matching close: first subsequent line where └ is at the same
      // codepoint column as ┌ AND the line contains ┘.
      let closeLi = -1;
      for (let lj = li + 1; lj < lines.length; lj++) {
        const candidate = lines[lj];
        if (!candidate.includes('└') || !candidate.includes('┘')) continue;
        const candidateLeftCol = charIndexOf(candidate, '└') + 1;
      ...
      for (let lj = li + 1; lj < closeLi; lj++) {
        ...
        const actualCol = lastCharIndexOf(innerLine, '│') + 1;
      ...
      const actualBotCol = lastCharIndexOf(closeLine, '┘') + 1;

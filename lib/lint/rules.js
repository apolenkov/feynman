...
function L06_priority_scale(ast) {
  ...
  const content = ast.content;
  const hasUp = content.includes('▲');
  const hasDown = content.includes('▼');
...
// ---------------------------------------------------------------------------
function L07_no_mermaid_mix(ast, fullText) {
  // ast may be null when called for full-text check
  if (!fullText) return [];

...
function renderDoctorReport(checks: string[], failCount: number): void {
  ...
  checks.forEach((line, i) => console.log(`│ ${line}${' '.repeat(Math.max(0, innerW - 1 - (stripped[i]?.length ?? 0)))}│`));
...
export function cmdDoctor(opts: { noExit?: boolean } = {}): void {
  ...
  if (settingsExists) {
    const hooks = readSettings()['hooks'] as Record<string, Array<Record<string, unknown>>> | undefined;
    const entry = (hooks?.['SessionStart'] ?? []).find(group => (group['hooks'] as Array<Record<string, unknown>> | undefined)?.some(h => isSessionStartHookCommand(h['command'])));
    const hook = (entry?.['hooks'] as Array<Record<string, unknown>> | undefined)?.find(h => isSessionStartHookCommand(h['command']));
    hookPath = hook ? extractHookScriptPath(String(hook['command']), 'feynman-session-start.ts') ?? extractHookScriptPath(String(hook['command']), 'feynman-session-start.js') : null;
  }
  check('hook registered (feynman-session-start in SessionStart)', hookPath !== null);
  let hookReadable = false;
  try { if (hookPath) fs.accessSync(hookPath, fs.constants.R_OK); hookReadable = hookPath !== null; } catch (_) { /* advisory check */ }
  check('session hook script file exists and is readable', hookReadable);
  let rulesOk = false;
  try { rulesOk = fs.statSync(RULES_PATH).size > 0; } catch (_) { /* advisory check */ }
  ...
  if (fs.existsSync(tc.statePath + '.bak')) check('state.json recovered from corruption (backup: state.json.bak)', true, true);
  const enabled = state?.['enabled'] === true;
  check(enabled ? '.feynman-active flag present when enabled' : '.feynman-active flag absent when disabled', enabled === fs.existsSync(tc.flagPath));

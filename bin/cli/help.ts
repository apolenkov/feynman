...
${c.bold('Commands:')}
  ...
  state        Show or change Feynman state
  status       Alias for feynman state status
...
export const INSTALL_HELP = `${c.bold('feynman install')} — register feynman hook in Codex
...
export const UNINSTALL_HELP = `${c.bold('feynman uninstall')} — remove Codex hook

${c.bold('Usage:')}
  feynman uninstall

Removes feynman hook entries from Codex config.
...
export const DOCTOR_HELP = `${c.bold('feynman doctor')} — check Codex installation health

${c.bold('Usage:')}
  feynman doctor

Checks:
  1. Codex hook config present
...
export const STATE_HELP = `${c.bold('feynman state')} — show or change Codex state

${c.bold('Usage:')}
  feynman state                  # show state
  feynman state on|start          # enable diagram assistance
  feynman state off|stop          # disable diagram assistance
  feynman state lite|full|ultra   # set diagram intensity
  feynman state style short|middle|full
  feynman status                  # alias for state status
`;

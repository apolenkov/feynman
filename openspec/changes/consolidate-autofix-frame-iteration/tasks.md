...
- [x] 1.1 In `tests/autofix.test.ts`, add a temporary characterisation test that
...
- [x] 2.1 In `tests/autofix.test.ts`, add the kept regression tests for the three
...
- [x] 2.2 Delete the temporary characterisation test from task 1.1.

## 3. Consolidate onto nextFrame (green)

- [x] 3.1 In `lib/lint/autofix.ts`, replace the hand-written frame-scanning loop
...
- [x] 3.2 Run `tests/autofix.test.ts`; the task-2.1 regression tests now pass.

## 4. Reconcile fixtures and close the gate

- [x] 4.1 Run the full `tests/autofix.test.ts` and `tests/lint.test.ts` suites.
...
- [x] 4.2 Run `npm run typecheck`, `npm run eslint`, `npm test`, `npm run test:docs`
  — all green.
- [x] 4.3 Commit as `refactor(lint): route autofix through nextFrame (one frame definition)`.

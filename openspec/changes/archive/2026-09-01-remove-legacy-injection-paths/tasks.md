...
- [x] 1.1 In `lib/feynman-state.ts`, make `readRulesForIntensity` XML-only: delete
...
- [x] 1.2 Update/remove the format tests: delete the activate-hook "Path 11:
...
- [x] 1.3 Apply the `rules-injection` spec delta for the format: the MODIFIED
...
- [x] 2.1 Delete `hooks/feynman-activate.ts` (this also removes `MALFORMED_FALLBACK`).
- [x] 2.2 In `lib/feynman-state.ts`, remove the `malformed_rules` field from the
  `FeynmanState` interface (set/cleared only on the deleted path). Keep the
  `count→injections` migration untouched.
- [x] 2.3 Repoint the hook path in `bin/cli/targets.ts` and `bin/commands/install.ts`:
...
- [x] 2.4 KEEP `isFeynmanHookCommand` in `bin/cli/settings.ts` matching
...
- [x] 2.5 Apply the rest of the `rules-injection` spec delta: REMOVED (UserPromptSubmit
...
- [x] 2.6 Amend `docs/adr/0003-sessionstart-over-userpromptsubmit-injection.md`
  with a dated note that the legacy UserPromptSubmit hook is now removed (ADR-0005
  amend-style). Commit.

## 3. Reference sweep and final gate

- [x] 3.1 Grep every reference to `feynman-activate` (the hook, not
...
- [x] 3.2 `git add` the new/changed files (drift-guard sees only git-tracked
...
- [x] 3.3 Reconcile the superseded commit `8ba73c9` on
  `chore/post-merge-debt-cleanup`: the branch is gone and the shared helper is
  present in the merged implementation, so no follow-up is required.

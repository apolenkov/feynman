...
(`engines.node >= 22.18`); the published tarball ships compiled `.js` with all
...
- Node baseline rose from 18 to **22.18** (required for direct `.ts` execution
  without the experimental type-stripping flag).
